...
namespace TechLibrary.Controllers.Tests
{
    ...
    public class BooksControllerTests
    {
        ...
        {
            //  Arrange
            _mockBookService.Setup(b => b.GetAllBooksAsync()).Returns(Task.FromResult(It.IsAny<List<Domain.Book>>()));
            var sut = new BooksController(_mockLogger.Object, _mockBookService.Object, _mockMapper.Object);

            //  Act
            var result = await sut.GetAllBooks(2);

            //  Assert
            _mockBookService.Verify(s => s.GetAllBooksAsync(), Times.Once, "Expected GetBooksAsync to have been called once");
        }
    }
}